    ...
    <v-card-text>
      <v-form ref="form">
        <div v-for="(alternativa, index) in alternativas" :key="index" class="d-flex align-center">
          <v-checkbox v-model="alternativa.correta" label="Correta"
            @change="alternativaCorretaChanged(index)"></v-checkbox>
          <v-text-field v-model="alternativa.enunciado" label="Enunciado da alternativa"></v-text-field>
        </div>
        <v-icon class="custom" color="blue" :class="{ 'animated-btn': animateButton }" @click="adicionarNovaAlternativa"
          @mouseover="animateButton = true" @mouseleave="resetAnimation">
          mdi-plus-circle
        </v-icon>
      </v-form>
    </v-card-text>
